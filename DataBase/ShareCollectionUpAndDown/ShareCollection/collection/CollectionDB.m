...
- (NSMutableArray *)selectDataFromDataBase {
    ...
    while ([result next]) {
        ...
        CollectionModel *model = [[[CollectionModel alloc] initWithID:ID title:title content:content] autorelease];
...
- (void)insertDataInDataBase:(CollectionModel *)model {
    ...
    BOOL isSuccess = [self.db executeUpdate:@"insert into Collection(c_title, c_content) values(? ,?)", model.title, model.content];
    NSLog(@"%@", isSuccess ? @"插入成功" : @"插入失败");
    //需要做一步数据的同步处理,同步唯一标识
    model.ID = self.db.lastInsertRowId;
